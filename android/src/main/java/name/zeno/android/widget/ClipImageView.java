...

{
  ...
  /** 水平方向与View的边距 */
  private int paddingHorizontal;//——屏幕边缘离截图区的宽度
  /** 垂直方向与View的边距 */
  private int paddingVertical;//——屏幕顶部离截图区的高度
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (dragEnable) {
          if (getDrawable() != null) {
            ...
            // 如果宽度小于屏幕宽度，则禁止左右移动
            if (rectF.width() <= getWidth() - paddingHorizontal * 2) {
              dx = 0;
            }
            // 如果高度小雨屏幕高度，则禁止上下移动
            if (rectF.height() <= getHeight() - paddingVertical * 2) {
              dy = 0;
            }
  ...
  public void onGlobalLayout()
  {
    if (once) {
      ...
      float scaleW = (float) (w - paddingHorizontal * 2) / dw;
      float scaleH = (float) (h - paddingVertical * 2) / dh;
      initScale = Math.max(scaleW, scaleH);

      SCALE_MID = initScale * 2;
      SCALE_MAX = initScale * 4;
      mScaleMatrix.postTranslate((w - dw) / 2, (h - dh) / 2);//平移至屏幕中心
      mScaleMatrix.postScale(initScale, initScale, w / 2, h / 2);//设置缩放比例
      // 图片移动至屏幕中心
      setImageMatrix(mScaleMatrix);
      once = false;
    }
  ...
  public Bitmap clip()
  {
    ...
    return Bitmap.createBitmap(bitmap, paddingHorizontal,
        paddingVertical, getWidth() - 2 * paddingHorizontal,
        getWidth() - 2 * paddingHorizontal);
  }
  ...
  private void checkBorder()
  {
    ...
    if (rect.width() + 0.01 >= width - 2 * paddingHorizontal) {
      ...
      if (rect.right < width - paddingHorizontal) {
        deltaX = width - paddingHorizontal - rect.right;
      }
    }
    if (rect.height() + 0.01 >= height - 2 * paddingVertical) {
      ...
      if (rect.bottom < height - paddingVertical) {
        deltaY = height - paddingVertical - rect.bottom;
      }
    }
