...
public class SideBar extends View
{
  ...
  protected void onDraw(Canvas canvas)
  {
    ...
    for (int i = 0; i < indexMap.size(); i++) {
      ...
      int colorId = i == choose ? R.color.colorPrimaryDark : R.color.md_grey700;
  ...
  private void initIndex(@NonNull RecyclerView.Adapter adapter)
  {
    ...
    for (int i = 0; i < adapter.getItemCount(); i++) {
      Index item = ((IndexAdapter) adapter).getItem(i);
      if (i == 0) {
        indexMap.put(i, item.getIndex());
      } else {
        Index preItem = ((IndexAdapter) adapter).getItem(i - 1);
