...
public class TintManager
{
  ...
  private static final String TAG = "EmTintManager";
  ...
  public static void tintDrawable(Drawable drawable, TintInfo tint, int[] state)
  {
    ...
    if (tint.hasTintList || tint.hasTintMode) {
      drawable.setColorFilter(createTintFilter(
          tint.hasTintList ? tint.tintList : null,
          tint.hasTintMode ? tint.tintMode : DEFAULT_MODE,
          state));
    } else {
  ...
  private static PorterDuffColorFilter createTintFilter(ColorStateList tint, PorterDuff.Mode tintMode, final int[] state)
  {
    ...
    final int color = tint.getColorForState(state, Color.RED);
